...
namespace HomeCenter.NET.Utilities
{
    public class Hook
    {
        ...
        private void SendEvent(int code, IntPtr wParam, ref Kbdllhookstruct lParam)
        {
            ...
            if (OneUpEvent)
            {
                if (LastState != null && LastState.Item1 == lParam.VkCode && LastState.Item2 == lParam.Flags)
                {
                    return;
                }
                LastState = new Tuple<uint, uint>(lParam.VkCode, lParam.Flags);
            }

            var isKeyDown = lParam.Flags >> 7 == 0;
            if (isKeyDown)
            {
                KeyDownEvent?.Invoke(this, new KeyboardHookEventArgs(lParam));
            }
            else
            {
                KeyUpEvent?.Invoke(this, new KeyboardHookEventArgs(lParam));
            }
        ...
        private int HookCallback(int code, IntPtr wParam, ref Kbdllhookstruct lParam)
        {
            ...
            try
            {
                SendEvent(code, wParam, ref lParam);
            }
            finally
            {
